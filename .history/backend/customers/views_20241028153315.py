...
def toggle_favorite(request, restaurant_id):
    ...
    # Check if restaurant is already in the customer's favorites
    if restaurant in customer.favorites.all():
        customer.favorites.remove(restaurant)
        is_favorite = False
        logger.info(f"{restaurant.restaurant_name} removed from {customer.username}'s favorites.")
    else:
        customer.favorites.add(restaurant)
        is_favorite = True
        logger.info(f"{restaurant.restaurant_name} added to {customer.username}'s favorites.")
