...
const Favorites = () => {
    ...
    useEffect(() => {
        ...
        if (token) {
            verifyToken(token);  // Verify token before fetching favorites
        } else {
    ...
    const verifyToken = async (token) => {
        try {
            // A lightweight request to verify the token
            await axios.get('http://localhost:8000/api/token/verify/', {
                headers: { Authorization: `Bearer ${token}` }
            });
            setIsLoggedIn(true);
            fetchFavorites(token);  // Fetch favorites if token is valid
        } catch (error) {
