...
class FavoriteRestaurantSerializer(serializers.ModelSerializer):
    class Meta:
        model = Restaurant
        fields = ['id', 'restaurant_name', 'description', 'location']

class CustomerFavoritesSerializer(serializers.ModelSerializer):
    favorites = FavoriteRestaurantSerializer(many=True, read_only=True)  # Serialize favorites with detailed info

    class Meta:
        model = Customer
        fields = [
            'username', 'name', 'email', 'date_of_birth', 'city', 'state', 
            'country', 'nickname', 'phone', 'profile_picture', 'favorites'
        ]
