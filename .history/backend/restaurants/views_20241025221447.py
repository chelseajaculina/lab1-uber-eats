...
class UploadRestaurantProfilePictureView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            # Ensure authenticated user is associated with the restaurant
            restaurant = Restaurant.objects.get(pk=request.user.pk)

            # Retrieve and validate 'profile_picture' in request data
            ...
            # Construct and return the absolute URL for the profile picture
...
class RestaurantDetailView(APIView):
 ...
 # Correct the usage of DishSerializer in the DishListCreateView
class DishListCreateView(APIView):
    parser_classes = [MultiPartParser, FormParser]
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        if request.user.user_type == 'restaurant':
            data = request.data.copy()
            serializer = DishSerializer(data=data, context={'request': request})  # Add context here
            if serializer.is_valid():
                serializer.save(restaurant=request.user)
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
