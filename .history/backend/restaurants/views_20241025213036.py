...
class UploadRestaurantProfilePictureView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            # Retrieve the restaurant instance based on the authenticated user
            restaurant = Restaurant.objects.get(pk=request.user.pk)

            # Check if profile_picture is explicitly set to null in the request data
            if 'profile_picture' in request.data and request.data['profile_picture'] is None:
                # Remove the profile picture if 'profile_picture' is set to null
            ...
            # Otherwise, update with a new profile picture if one is provided
            ...
            # Correctly build the absolute URL for the profile picture
