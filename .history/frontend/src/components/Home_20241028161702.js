...
const Home = () => {
  ...
  // Toggle favorite status, update backend and localStorage
  const toggleFavorite = (restaurantId, restaurantName) => {
    const isFavorite = favorites[restaurantId];
    const updatedFavorites = { ...favorites, [restaurantId]: !isFavorite };

    // Update the backend
    if (!isFavorite) {
      axios.post('http://localhost:8000/api/customers/favorites/', { restaurant_name: restaurantName })
        .then(response => {
      ...
      axios.delete(`http://localhost:8000/api/customers/favorites/${restaurantId}/`)
        .then(response => {
            ...
            <div key={index} className="restaurant-card">
              ...
              <div className="image-container">
                ...
                <button 
                  ...
                  onClick={() => toggleFavorite(item.restaurant.id, item.restaurant.restaurant_name)}
                >
