...
const RestaurantMenu = () => {
  ...
  const [existingImage, setExistingImage] = useState(null); // Track the existing image URL for editing
  ...
  const handleOpenModal = (dish = null) => {
    if (dish) {
      ...
      setDishData({
        ...
        image: null, // Image will be uploaded only if updated
      });
      setExistingImage(dish.image); // Set the existing image URL
    } else {
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form className="dish-form" onSubmit={handleSubmit}>
              ...
              {existingImage && !dishData.image && (
                <div>
                  <label>Current Image:</label>
                  <img src={`http://localhost:8000${existingImage}`} alt="Current Dish" className="current-dish-image" />
                </div>
              )}

              <div>
                <label>Change dish image:</label>
