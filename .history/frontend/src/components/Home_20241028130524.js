...
const Home = () => {
  ...
  const authToken = localStorage.getItem('access_token'); // Assumes auth token is stored here
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/api/favorites/', {
      ...
      .then(response => {
        const favoriteIds = response.data.map(fav => fav.restaurant_id);
        const favoritesMap = {};
  ...
  // Toggle favorite status, update state, and sync with the backend
  const toggleFavorite = (restaurantId) => {
    const isFavorited = favorites[restaurantId];

    if (isFavorited) {
      // Remove favorite from the backend
      axios.delete(`http://localhost:8000/api/favorites/remove/${restaurantId}/`, {
        headers: { Authorization: `Bearer ${authToken}` }
      })
      .then(() => {
        setFavorites(prevFavorites => {
          const updatedFavorites = { ...prevFavorites };
          delete updatedFavorites[restaurantId];
      ...
      // Add favorite to the backend
      axios.post('http://localhost:8000/api/favorites/add/', { restaurant_id: restaurantId }, {
        headers: { Authorization: `Bearer ${authToken}` }
      })
      .then(() => {
        setFavorites(prevFavorites => ({
