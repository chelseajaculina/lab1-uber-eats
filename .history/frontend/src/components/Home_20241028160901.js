...
const toggleFavorite = (restaurantName) => {
  axios.post(`http://localhost:8000/api/customers/toggle-favorite/${restaurantName}/`, {}, {
    headers: {
      'X-CSRFToken': getCsrfToken(),
      'Origin': 'http://localhost:3000',  // Explicitly set the origin
    }
  })
  .then(response => {
  ...
  onClick={() => toggleFavorite(item.restaurant.restaurant_name)}
>
  {favorites[item.restaurant.restaurant_name] ? <FaHeart className="heart-icon filled" /> : <FaRegHeart className="heart-icon" />}
</button>
